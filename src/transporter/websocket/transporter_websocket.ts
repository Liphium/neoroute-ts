            ...
            try {
                ...
                const timeoutId = setTimeout(() => {
                    if (this.conn?.readyState !== WebSocket.OPEN) {
                        this.conn?.close();
                        reject(new Error("failed to connect to websocket server: connection timeout"));
                    }
                }, 20000);
                        ...
                        this.sendMutex = this.sendMutex.then(async () => {
                            return new Promise<void>((res, rej) => {
                                if (!this.conn || this.conn.readyState !== WebSocket.OPEN) {
                                    return rej(new Error("websocket connection is not open"));
                                }
                                this.conn.send(data as unknown as BufferSource);
                                res();
                            });
                        });
                ...
                this.conn.onerror = (event) => {
                    clearTimeout(timeoutId);
                    reject(new Error(`failed to connect to websocket server or an error occurred`));
                }

                this.conn.onclose = (event) => {
                    logger.info("websocket connection closed by remote", event.code, event.reason);
                }
                ...
                this.conn.onmessage = async (event) => {
                    ...
                    if (event.data instanceof ArrayBuffer) {
                        ...
                        msg = new Uint8Array(event.data.buffer, event.data.byteOffset, event.data.byteLength);
                    } else {
                        logger.info("wrong message type received:", typeof event.data);
                        return;
                    }
